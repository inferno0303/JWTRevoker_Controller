...
class ClusterTraceMicroservicesV2022NodeMetrics(Base):
    ...
    def __repr__(self):
        return f"<cluster_trace_microservices_v2022_node_metrics id: {self.id}, timestamp: {self.timestamp}, nodeid: {self.nodeid}, http_mcr: {self.http_mcr}>"
...
class IWQoS23EdgeMeasurements(Base):
    ...
    def __repr__(self):
        return (f"<iwqos23_edge_measurements id: {self.id}, "
                f"src_machine_id: {self.src_machine_id}, "
                f"dst_machine_id: {self.dst_machine_id}, "
                f"tcp_out_delay: {self.tcp_out_delay}, "
...
class EdgeTable(Base):
    ...
    def __repr__(self):
        return (f"<edge_table id: {self.id}, "
                f"time_sequence: {self.time_sequence}, "
                f"src_node: {self.src_machine_id}, "
                f"dst_node: {self.dst_machine_id},"
                f" tcp_out_delay: {self.tcp_out_delay}, "
                f"tcp_out_packet_loss: {self.tcp_out_packet_loss}>")
...
class NodeTable(Base):
    ...
    def __repr__(self):
        return (f"<node_table id: {self.id}, "
                f"time_sequence: {self.time_sequence}, "
                f"nodeid: {self.nodeid}, "
                f"cpu_utilization: {self.cpu_utilization}, "
                f"memory_utilization: {self.memory_utilization}")


class NodeTablePrediction(Base):
    __tablename__ = "node_table_prediction"
    ...
    def __repr__(self):
        return (f"<node_table_prediction id: {self.id}, "
                f"time_sequence: {self.time_sequence}, "
                f"nodeid: {self.nodeid}, "
                f"cpu_utilization: {self.cpu_utilization}")
